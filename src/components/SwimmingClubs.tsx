...
// Define the SwimmingClub interface
interface SwimmingClub {
  name: string;
  description: string;
  instagram: string;
  website: string;
...
const SwimmingClubs = () => {
  const [clubs, setClubs] = useState<SwimmingClub[]>([]);
  ...
  useEffect(() => {
    ...
    const RANGE = 'SwimmingClubs!A1:N12';
    ...
    .then(data => {
      ...
      const clubsData = rows.slice(1).map((row: any[]) => {
        ...
        return {
          ...
          types: row[13] ? row[13].split(',').map((type: string) => type.trim()) : [],
        };
      });
      setClubs(clubsData);
      setAreas(Array.from(new Set(clubsData.map((club: SwimmingClub) => club.area))));
    })
    .catch(error => console.error('Error fetching data:', error));
  }, []);
    

  const filteredClubs = clubs.filter((club: SwimmingClub) => {
    ...
    const isTypeSelected = selectedTypes.length === 0 || selectedTypes.every(type => club.types.includes(type));
  
    return isFeeAcceptable && isAreaSelected && isDaySelected && isFilterBeginnerFriendly && isFilterSocial && isFilterVerySocial && isTypeSelected;
  });
  ...
  const handleTypeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setSelectedTypes(selectedTypes.includes(type) ? selectedTypes.filter(d => d !== type) : [...selectedTypes, type]);
  };

  return (
    <div>
      <h1>Swimming Clubs</h1>
      <p>This page shows some of the active swimming clubs in the Bay Area, but unfortunately I don't think it's comprehensive.
        I have added some filters, so that you can find a club that matches what you're looking for, but the information might not be up-to-date. Clear the filters and scroll down to see all of the clubs. Please contact me if you see any errors.
      </p>
  ...
  <div className="days-checkboxes">
    {["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Periodic"].map(day => (
      <label key={day} className="day-checkbox">
        ...
        <label className="filter-label"><strong>Type:</strong></label>
  <div className="types-checkboxes">
          {["Pool", "Synchronized", "Open water"].map(type => (
...
<div className="social-filter">
  <label>
  <strong>Social</strong> (regular socializing after swims):
    <input 
      type="checkbox" 
      checked={isSocial} 
...
<div className="very-social-filter">
  <label>
  <strong>Very social</strong> (the social is more important than the swim):
    <input 
      type="checkbox" 
...
<ul>
  {filteredClubs.map((club, index) => (
    <li key={index} className="club-listing">
      ...
      <div><strong>Types:</strong> {club.types.join(', ')}</div>
